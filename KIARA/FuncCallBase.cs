...
namespace KIARA
{
    ...
    public abstract class FuncCallBase : IClientFunctionCall
    {
        ...
        public IClientFunctionCall OnSuccess<T>(Action<T> handler)
        {
        ...
        public IClientFunctionCall OnSuccess(Action handler)
        {
            if (state == State.InProgress)
                successHandlers.Add(handler);
            else if (state == State.Success)
        ...
        public IClientFunctionCall OnException(Action<Exception> handler)
        {
            if (state == State.InProgress)
        ...
        public IClientFunctionCall OnError(Action<string> handler)
        {
            if (state == State.InProgress)
        ...
        public IClientFunctionCall OnFailure(Action handler)
        {
            if (state == State.InProgress)
            {
